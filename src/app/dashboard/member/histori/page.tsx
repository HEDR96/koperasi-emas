  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ color:"#2D1B00", fontSize:"1.4rem", fontWeight:700, margin:0 }}>Histori Transaksi</h1>
          <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".85rem", margin:"4px 0 0" }}>Semua transaksi dan permintaan Anda</p>
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:12 }}>
        {[
          { label:"Total",    value:stats.total,     color:"#2D1B00",    bg:"rgba(255,255,255,0.04)" },
          { label:"Pending",  value:stats.pending,   color:"#92400e", bg:"rgba(251,191,36,0.08)" },
          { label:"Selesai",  value:stats.completed, color:"#065f46", bg:"rgba(52,211,153,0.08)" },
          { label:"Ditolak",  value:stats.rejected,  color:"#991b1b", bg:"rgba(153,27,27,0.06)" },
        ].map(s => (
          <div key={s.label} style={{ background:s.bg, border:`1px solid ${s.color}22`, borderRadius:12, padding:"14px 16px" }}>
            <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".73rem", margin:"0 0 5px" }}>{s.label}</p>
      ...
      {loading ? (
        ...
        <div style={{ background:"rgba(255,255,255,0.72)", border:"1px solid rgba(201,162,39,0.12)", borderRadius:14, padding:"48px", textAlign:"center" }}>
          <ArrowLeftRight style={{ width:32, height:32, color:"rgba(201,162,39,0.2)", margin:"0 auto 12px" }} />
          <p style={{ color:"rgba(101,67,14,0.4)", fontSize:".9rem", margin:0 }}>Belum ada transaksi.</p>
        </div>
      ) : (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {filtered.map((tx, i) => (
            <motion.div key={tx.id} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*0.03 }}
              style={{ background:"rgba(255,255,255,0.72)", border:"1px solid rgba(201,162,39,0.12)", borderRadius:14, padding:"16px 18px" }}>
              <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12, flexWrap:"wrap" }}>
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:5, flexWrap:"wrap" }}>
                    <p style={{ color:"#2D1B00", fontWeight:700, fontSize:".92rem", margin:0 }}>{TYPE_LABEL[tx.type]||tx.type}</p>
                    <span style={{ background:STATUS_BG[tx.status]||"rgba(201,162,39,0.15)", color:STATUS_COLOR[tx.status]||"#fff", borderRadius:6, padding:"2px 8px", fontSize:".72rem", fontWeight:700, display:"flex", alignItems:"center", gap:4 }}>
                  ...
                  {tx.status==="rejected" && tx.notes && (
                    <p style={{ color:"#991b1b", fontSize:".78rem", marginTop:6, background:"rgba(248,113,113,0.06)", borderRadius:6, padding:"5px 10px" }}>
                      Alasan: {tx.notes}
                    </p>
                  )}
                </div>
                <p style={{ color:"#8B6010", fontWeight:800, fontSize:"1rem", margin:0, flexShrink:0 }}>{fmt(tx.amount)}</p>
              </div>
            </motion.div>
          ))}
        </div>
