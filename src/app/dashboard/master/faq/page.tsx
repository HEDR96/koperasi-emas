  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24, maxWidth:860 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ color:"#2D1B00", fontSize:"1.4rem", fontWeight:700, margin:0 }}>Kelola FAQ</h1>
          <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".85rem", margin:"4px 0 0" }}>Edit, tambah, dan hapus pertanyaan umum di landing page</p>
        </div>
      ...
      <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div>
            <label style={{ color:"rgba(101,67,14,0.45)", fontSize:".78rem", display:"block", marginBottom:6 }}>Pertanyaan</label>
          ...
          <div>
            <label style={{ color:"rgba(101,67,14,0.45)", fontSize:".78rem", display:"block", marginBottom:6 }}>Jawaban</label>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            <div>
              <label style={{ color:"rgba(101,67,14,0.45)", fontSize:".78rem", display:"block", marginBottom:6 }}>Urutan Tampil</label>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {items.map((item, i) => (
            <motion.div key={item.id} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*.04 }}
              style={{ background:"rgba(255,255,255,0.72)", border:`1px solid ${item.is_active?"rgba(212,175,55,0.12)":"rgba(201,162,39,0.1)"}`, borderRadius:13, padding:"16px 18px" }}>
              <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                    <span style={{ color:"rgba(101,67,14,0.35)", fontSize:".72rem", background:"rgba(255,255,255,0.72)", padding:"2px 8px", borderRadius:6 }}>#{item.urutan}</span>
                    {!item.is_active && <span style={{ background:"rgba(153,27,27,0.1)", color:"#991b1b", fontSize:".7rem", padding:"2px 8px", borderRadius:6 }}>Disembunyikan</span>}
                  </div>
                  <p style={{ color:"#2D1B00", fontWeight:600, fontSize:".9rem", marginBottom:6 }}>{item.pertanyaan}</p>
                  <p style={{ color:"rgba(101,67,14,0.6)", fontSize:".82rem", lineHeight:1.6, margin:0 }}>{item.jawaban}</p>
                </div>
