...
// Master member page reuses the same full-featured component as admin
// but master can also see admin accounts
import { useState, useEffect, useCallback } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Search, UserCheck, UserX, Eye, X, Coins, PiggyBank, CreditCard, TrendingUp, Download } from "lucide-react";
import { formatDate, formatCurrency } from "@/lib/utils";
import { supabase } from "@/lib/supabase";

type Member = {
  id: string; name: string; email: string | null; phone: string | null;
  gold_grams: number; status: string; role: string; created_at: string;
};
...
export default function MasterMemberPage() {
  ...
  const load = useCallback(async () => {
    ...
    let q = (supabase.from("profiles") as any)
      .select("id, name, email, phone, gold_grams, status, role, created_at")
      .not("role", "eq", "master")
  ...
  const counts = {
    ...
    thisMonth: members.filter(m=>new Date(m.created_at)>new Date(Date.now()-30*86400000)).length,
  };
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:24 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:16 }}>
        {[
          ...
          { label:"Aktif",        value:counts.active,    color:"#4ade80" },
          { label:"Menunggu",     value:counts.pending,   color:"#fb923c" },
          { label:"Suspend",      value:counts.suspended, color:"#f87171" },
          { label:"Baru 30 Hari", value:counts.thisMonth, color:"#60a5fa" },
        ].map(s => (
      ...
      <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}
        ...
        <div style={{ padding:"14px 18px", borderBottom:"1px solid rgba(255,255,255,0.05)", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {/* Role filter */}
            {(["member","admin","all"] as const).map(r => (
              <button key={r} onClick={()=>{ setRoleFilter(r); setPage(1); }}
                style={{ padding:"4px 10px", borderRadius:20, border:"1px solid", fontSize:".72rem", fontWeight:600, cursor:"pointer",
                  borderColor: roleFilter===r?"#60a5fa":"rgba(255,255,255,0.08)",
                  background:  roleFilter===r?"rgba(96,165,250,0.1)":"transparent",
                  color:       roleFilter===r?"#60a5fa":"rgba(255,255,255,0.3)" }}>
                {r==="all"?"Semua Role":r==="admin"?"Admin":"Member"}
              </button>
            ))}
            <div style={{ width:1, height:20, background:"rgba(255,255,255,0.08)", alignSelf:"center", margin:"0 2px" }} />
            {(["all","pending","active","suspended"] as const).map(f => (
              <button key={f} onClick={()=>{ setFilter(f); setPage(1); }}
                style={{ padding:"4px 10px", borderRadius:20, border:"1px solid", fontSize:".72rem", fontWeight:600, cursor:"pointer",
                  borderColor: filter===f?"#D4AF37":"rgba(255,255,255,0.08)",
                  background:  filter===f?"rgba(212,175,55,0.1)":"transparent",
                  color:       filter===f?"#D4AF37":"rgba(255,255,255,0.3)" }}>
                {f==="all"?"Semua Status":f==="pending"?"Menunggu":f==="active"?"Aktif":"Suspend"}
              </button>
            ))}
          </div>
          <div style={{ display:"flex", gap:8 }}>
            ...
            <button className="btn-outline-gold" style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 12px", borderRadius:9, cursor:"pointer", fontSize:".75rem" }}>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead>
              <tr style={{ borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
                {["Member","Email","Role","Emas","Bergabung","Status","Aksi"].map(h => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={7} style={{ padding:40, textAlign:"center", color:"rgba(255,255,255,0.25)", fontSize:".83rem" }}>Memuat data...</td></tr>
              ) : paged.length===0 ? (
                <tr><td colSpan={7} style={{ padding:40, textAlign:"center", color:"rgba(255,255,255,0.25)", fontSize:".83rem" }}>Tidak ada data</td></tr>
              ) : paged.map(m => {
                  ...
                  <tr key={m.id} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                    <td style={{ padding:"11px 14px" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        <div className="bg-gold-gradient" style={{ width:28, height:28, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:800, color:"#0a0a0a", fontSize:".7rem", flexShrink:0 }}>{m.name?.[0]||"?"}</div>
                        <div>
                          <p style={{ color:"#fff", fontSize:".81rem", fontWeight:600 }}>{m.name}</p>
                          <p style={{ color:"rgba(255,255,255,0.25)", fontSize:".69rem" }}>{m.phone||"-"}</p>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding:"11px 14px", color:"rgba(255,255,255,0.45)", fontSize:".78rem" }}>{m.email}</td>
                    <td style={{ padding:"11px 14px" }}>
                      <span style={{ background: m.role==="admin"?"rgba(96,165,250,0.1)":"rgba(212,175,55,0.08)", color: m.role==="admin"?"#60a5fa":"#D4AF37", borderRadius:8, padding:"2px 8px", fontSize:".7rem", fontWeight:700 }}>
                        {m.role}
                      </span>
                    </td>
                    <td style={{ padding:"11px 14px", color:"#D4AF37", fontWeight:700, fontSize:".81rem" }}>{(m.gold_grams||0).toFixed(2)}g</td>
                    <td style={{ padding:"11px 14px", color:"rgba(255,255,255,0.3)", fontSize:".76rem" }}>{formatDate(m.created_at)}</td>
                    <td style={{ padding:"11px 14px" }}>
                      <span style={{ background:s.bg, color:s.color, borderRadius:8, padding:"3px 9px", fontSize:".72rem", fontWeight:700 }}>{s.label}</span>
                    </td>
                    <td style={{ padding:"11px 14px" }}>
        ...
        {pages>1 && (
          <div style={{ padding:"12px 18px", borderTop:"1px solid rgba(255,255,255,0.05)", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
            <p style={{ color:"rgba(255,255,255,0.25)", fontSize:".78rem" }}>{filtered.length} records</p>
            <div style={{ display:"flex", gap:5 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                  <div style={{ background:"rgba(192,132,252,0.07)", border:"1px solid rgba(192,132,252,0.2)", borderRadius:14, padding:16 }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
                      {[{l:"Pokok",v:formatCurrency(detail.simpanan.pokok),c:"#D4AF37"},{l:"Wajib",v:formatCurrency(detail.simpanan.wajib),c:"#60a5fa"},{l:"Total",v:formatCurrency(detail.simpanan.total),c:"#c084fc"}].map(s=>(
                        <div key={s.l} style={{ textAlign:"center", background:"rgba(255,255,255,0.03)", borderRadius:10, padding:"10px 6px" }}>
                          <p style={{ color:"rgba(255,255,255,0.4)", fontSize:".7rem", marginBottom:4 }}>{s.l}</p>
                          <p style={{ color:s.c, fontWeight:800, fontSize:".82rem" }}>{s.v}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ background:"rgba(212,175,55,0.06)", border:"1px solid rgba(212,175,55,0.18)", borderRadius:14, padding:16 }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
                      {[{l:"Beli",v:`${detail.transaksi.beli}x`,c:"#4ade80"},{l:"Buyback",v:`${detail.transaksi.buyback}x`,c:"#fb923c"},{l:"Vol. Beli",v:`${detail.transaksi.totalGram.toFixed(2)}g`,c:"#D4AF37"}].map(s=>(
                        <div key={s.l} style={{ textAlign:"center", background:"rgba(255,255,255,0.03)", borderRadius:10, padding:"10px 6px" }}>
                          <p style={{ color:"rgba(255,255,255,0.4)", fontSize:".7rem", marginBottom:4 }}>{s.l}</p>
                          <p style={{ color:s.c, fontWeight:800, fontSize:".82rem" }}>{s.v}</p>
                        </div>
