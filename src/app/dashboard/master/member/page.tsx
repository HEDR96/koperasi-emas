  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ color:"#2D1B00", fontSize:"1.4rem", fontWeight:700, margin:0 }}>Kelola Member</h1>
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={()=>{ setShowForm(true); setError(""); setSuccess(""); setForm(DEFAULT_FORM); }}
            style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(6,95,70,0.09)", border:"1px solid rgba(52,211,153,0.25)", borderRadius:10, padding:"8px 16px", color:"#065f46", cursor:"pointer", fontSize:".85rem", fontWeight:600 }}>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} style={{ padding:"40px", textAlign:"center", color:"rgba(101,67,14,0.35)" }}>Tidak ada member{search?" yang cocok":""}.</td></tr>
              ) : filtered.map(m=>(
                <tr key={m.id} style={{ borderBottom:"1px solid rgba(201,162,39,0.1)" }}
                  onMouseEnter={e=>(e.currentTarget as HTMLElement).style.background="rgba(201,162,39,0.04)"}
                  onMouseLeave={e=>(e.currentTarget as HTMLElement).style.background="transparent"}
                >
                  <td style={{ padding:"13px 18px" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <div style={{ width:32, height:32, borderRadius:8, background:"rgba(6,95,70,0.09)", display:"flex", alignItems:"center", justifyContent:"center", color:"#065f46", fontWeight:700, fontSize:".85rem", flexShrink:0 }}>
                        {m.name?.[0]||"M"}
                      </div>
                      <span style={{ color:"#2D1B00", fontWeight:600, fontSize:".88rem" }}>{m.name}</span>
                      {m.role && m.role !== "member" && (
                        <span style={{ background:"rgba(212,175,55,0.14)", color:"#8B6010", borderRadius:6, padding:"2px 8px", fontSize:".68rem", fontWeight:600 }}>
                          {m.role === "master" ? "Master" : "Admin"}
                        </span>
                      )}
                    </div>
                  </td>
                  <td style={{ padding:"13px 18px", color:"rgba(101,67,14,0.55)", fontSize:".83rem" }}>{m.nik||"—"}</td>
                  <td style={{ padding:"13px 18px", color:"rgba(101,67,14,0.55)", fontSize:".83rem" }}>{m.phone||"—"}</td>
                  <td style={{ padding:"13px 18px" }}>
                    <span style={{ background:STATUS_BG[m.status]||"rgba(201,162,39,0.15)", color:STATUS_COLOR[m.status]||"#fff", borderRadius:6, padding:"3px 10px", fontSize:".75rem", fontWeight:600, textTransform:"capitalize" }}>
                      {m.status}
                    </span>
                  </td>
                  <td style={{ padding:"13px 18px", color:"#8B6010", fontWeight:600, fontSize:".88rem" }}>
                    {Number(m.gold_grams).toFixed(1)}
                  </td>
                  <td style={{ padding:"13px 18px", color:"rgba(101,67,14,0.7)", fontSize:".83rem" }}>
                    ...
                    <div style={{ display:"flex", gap:6 }}>
                      ...
                      {m.status==="pending" && (
                        <button onClick={()=>updateStatus(m.id,"active")}
                          style={{ background:"rgba(6,95,70,0.08)", border:"1px solid rgba(52,211,153,0.2)", borderRadius:8, padding:"5px 10px", color:"#065f46", cursor:"pointer", fontSize:".78rem", fontWeight:600 }}>
                          Setujui
                        </button>
                      )}
                      {m.status==="active" && (
                        <button onClick={()=>updateStatus(m.id,"suspended")}
                          style={{ background:"rgba(153,27,27,0.06)", border:"1px solid rgba(248,113,113,0.2)", borderRadius:8, padding:"5px 10px", color:"#991b1b", cursor:"pointer", fontSize:".78rem", fontWeight:600 }}>
                          Suspend
                        </button>
                      )}
                      {(m.status==="suspended" || m.status==="rejected") && (
                        <button onClick={()=>updateStatus(m.id,"active")}
                          style={{ background:"rgba(52,211,153,0.08)", border:"1px solid rgba(52,211,153,0.2)", borderRadius:8, padding:"5px 10px", color:"#065f46", cursor:"pointer", fontSize:".78rem", fontWeight:600 }}>
                          Aktifkan
                        </button>
                      )}
                    </div>
        ...
        {detail && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            ...
            <motion.div onClick={e=>e.stopPropagation()}
              ...
              style={{ width:"min(560px,96vw)", background:"rgba(255,252,220,0.95)", border:"1px solid rgba(212,175,55,0.2)", borderRadius:22, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 32px 80px rgba(0,0,0,0.6)" }}
            >
              {/* Modal Header */}
              <div style={{ padding:"20px 22px 16px", borderBottom:"1px solid rgba(201,162,39,0.12)", display:"flex", alignItems:"center", gap:14 }}>
                <div style={{ width:44, height:44, borderRadius:12, background:"linear-gradient(135deg,rgba(212,175,55,0.3),rgba(212,175,55,0.1))", display:"flex", alignItems:"center", justifyContent:"center", color:"#8B6010", fontWeight:800, fontSize:"1.1rem", flexShrink:0 }}>
                  {detail.name?.[0]?.toUpperCase()||"M"}
                </div>
                <div style={{ flex:1 }}>
                  <h2 style={{ color:"#2D1B00", fontWeight:700, fontSize:"1.05rem", margin:0 }}>{detail.name}</h2>
                  <span style={{ background:STATUS_BG[detail.status]||"rgba(201,162,39,0.15)", color:STATUS_COLOR[detail.status]||"#fff", borderRadius:5, padding:"2px 8px", fontSize:".72rem", fontWeight:600, textTransform:"capitalize" }}>{detail.status}</span>
                </div>
                <button onClick={()=>setDetail(null)}
                  style={{ background:"rgba(255,255,255,0.72)", border:"none", borderRadius:8, width:32, height:32, display:"flex", alignItems:"center", justifyContent:"center", color:"rgba(101,67,14,0.55)", cursor:"pointer", flexShrink:0 }}>
              ...
              <div style={{ padding:"18px 22px 24px" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:18 }}>
                  <div style={{ background:"rgba(212,175,55,0.07)", border:"1px solid rgba(212,175,55,0.2)", borderRadius:12, padding:"14px 16px" }}>
                    <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".73rem", margin:"0 0 5px" }}>Total Emas</p>
                    <p style={{ color:"#8B6010", fontWeight:800, fontSize:"1.3rem", margin:0 }}>{Number(detail.gold_grams).toFixed(1)}<span style={{ fontSize:".78rem", fontWeight:500, marginLeft:4 }}>gram</span></p>
                  </div>
                  <div style={{ background:"rgba(96,165,250,0.07)", border:"1px solid rgba(96,165,250,0.15)", borderRadius:12, padding:"14px 16px" }}>
                    <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".73rem", margin:"0 0 5px" }}>Saldo Rupiah</p>
                    <p style={{ color:"#1d4ed8", fontWeight:800, fontSize:"1.1rem", margin:0 }}>{fmt(detail.rupiah_balance)}</p>
                  </div>
                </div>

                {/* Info rows */}
                <div style={{ display:"flex", flexDirection:"column", gap:0, marginBottom:18, background:"rgba(255,255,255,0.72)", borderRadius:12, overflow:"hidden", border:"1px solid rgba(201,162,39,0.12)" }}>
                  {[
                    ["ID",        detail.nik||"—"],
                    ["Nomor HP",  detail.phone||"—"],
                    ["Bergabung", fmtDate(detail.created_at)],
                  ].map(([k,v], i, arr)=>(
                    <div key={k} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"11px 16px", borderBottom: i < arr.length-1 ? "1px solid rgba(201,162,39,0.1)" : "none" }}>
                      <span style={{ color:"rgba(101,67,14,0.45)", fontSize:".83rem" }}>{k}</span>
                      <span style={{ color:"#2D1B00", fontWeight:600, fontSize:".83rem" }}>{v}</span>
                    </div>
                  ))}
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"11px 16px" }}>
                    <span style={{ color:"rgba(101,67,14,0.45)", fontSize:".83rem" }}>Kuota Produk (Tambah Produk)</span>
                    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                      <input type="number" min={0} value={quotaInput} onChange={e=>setQuotaInput(e.target.value)}
                        style={{ width:56, background:"rgba(255,255,255,0.72)", border:"1px solid rgba(201,162,39,0.22)", borderRadius:7, padding:"5px 8px", color:"#2D1B00", fontSize:".82rem", textAlign:"center", outline:"none" }} />
                      <button onClick={()=>updateProductQuota(detail)} disabled={savingQuota || Number(quotaInput)===detail.product_quota}
                        style={{ background:"rgba(212,175,55,0.12)", border:"1px solid rgba(212,175,55,0.3)", borderRadius:7, padding:"5px 10px", color:"#8B6010", cursor: savingQuota||Number(quotaInput)===detail.product_quota ? "not-allowed" : "pointer", fontSize:".76rem", fontWeight:700, opacity: savingQuota||Number(quotaInput)===detail.product_quota ? .5 : 1 }}>
                        {savingQuota ? "..." : "Simpan"}
                      </button>
                    </div>
                  </div>
                ...
                return (
                  <>
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
                      <h3 style={{ color:"rgba(101,67,14,0.7)", fontSize:".8rem", fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", margin:0 }}>Simpanan</h3>
                      <span style={{ color:"#a78bfa", fontWeight:700, fontSize:".88rem" }}>{fmt(totalSim)}</span>
                    </div>
                    {buybackHarga > 0 && totalSim > 0 && (
                      <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".76rem", margin:"0 0 10px", textAlign:"right" }}>
                        Setara <strong style={{ color:"#8B6010" }}>{gram.toFixed(1)} gram</strong>
                        <span style={{ color:"rgba(101,67,14,0.35)" }}> · harga gadai {fmt(buybackHarga)}/gr</span>
                      </p>
                    )}
                    {totalSim > 0 && (
                      activeGadai ? (
                        <div style={{ background:"rgba(153,27,27,0.06)", border:"1px solid rgba(248,113,113,0.2)", borderRadius:10, padding:"10px 14px", marginBottom:14 }}>
                          <p style={{ color:"#991b1b", fontSize:".8rem", fontWeight:600, margin:0 }}>⚠ Ada gadai aktif — belum bisa gadai baru</p>
                          <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".75rem", margin:"3px 0 0" }}>Pinjaman {fmt(activeGadai.dana_cair)} · sisa {fmt(activeGadai.sisa_tagihan)} · {activeGadai.tenor} bln</p>
                        </div>
                      ) : (
                        <div style={{ background:"rgba(52,211,153,0.06)", border:"1px solid rgba(52,211,153,0.2)", borderRadius:10, padding:"10px 14px", marginBottom:10 }}>
                          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, flexWrap:"wrap" }}>
                            <div>
                              <p style={{ color:"#065f46", fontSize:".8rem", fontWeight:600, margin:0 }}>Dapat digadai hingga {fmt(maxPin)}</p>
                              <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".75rem", margin:"3px 0 0" }}>≈ {gram.toFixed(1)} gram · tenor 1-4 bulan</p>
                            </div>
                            {!gForm.open && (
                              <button onClick={()=>setGForm({ open:true, pinjaman:"", tenor:1, saving:false, err:"" })}
                                style={{ background:"rgba(29,78,216,0.1)", border:"1px solid rgba(96,165,250,0.35)", borderRadius:8, padding:"7px 14px", color:"#1d4ed8", cursor:"pointer", fontSize:".8rem", fontWeight:600 }}>
                                Ajukan Gadai
                              </button>
                            )}
                          </div>
                          {gForm.open && (() => {
                            ...
                            <div style={{ marginTop:12, display:"flex", flexDirection:"column", gap:10 }}>
                              {gForm.err && <p style={{ color:"#991b1b", fontSize:".78rem", margin:0 }}>{gForm.err}</p>}
                              <div>
                                <label style={{ color:"rgba(101,67,14,0.55)", fontSize:".75rem", display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                                  <span>Jumlah Pinjaman (maks {fmt(maxPin)})</span>
                                  <button onClick={()=>setGForm(f=>({...f,pinjaman:String(maxPin)}))} style={{ background:"none", border:"none", color:"#8B6010", cursor:"pointer", fontSize:".74rem" }}>Maks</button>
                                </label>
                                <div style={{ position:"relative" }}>
                                  <span style={{ position:"absolute", left:10, top:"50%", transform:"translateY(-50%)", color:"rgba(101,67,14,0.45)", fontSize:".8rem", pointerEvents:"none" }}>Rp</span>
                              ...
                              <div>
                                <label style={{ color:"rgba(101,67,14,0.55)", fontSize:".75rem", display:"block", marginBottom:5 }}>Tenor (bulan)</label>
                                <div style={{ display:"flex", gap:6 }}>
                                  {[1,2,3,4].map(t=>(
                                    <button key={t} onClick={()=>setGForm(f=>({...f,tenor:t}))}
                                      style={{ flex:1, padding:"8px", borderRadius:8, fontWeight:700, fontSize:".85rem", cursor:"pointer", border: gForm.tenor===t?"1px solid #60a5fa":"1px solid rgba(201,162,39,0.2)", background: gForm.tenor===t?"rgba(29,78,216,0.1)":"rgba(255,255,255,0.04)", color: gForm.tenor===t?"#60a5fa":"rgba(101,67,14,0.55)" }}>{t}</button>
                                  ))}
                                </div>
                              </div>
                              {pinRp > 0 && (
                                <div style={{ background:"rgba(96,165,250,0.07)", border:"1px solid rgba(96,165,250,0.2)", borderRadius:9, padding:"10px 12px", fontSize:".8rem" }}>
                                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                                    <span style={{ color:"rgba(101,67,14,0.55)" }}>Dana cair</span>
                                    <strong style={{ color:"#2D1B00" }}>{fmt(pinRp)}</strong>
                                  </div>
                                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                                    <span style={{ color:"rgba(101,67,14,0.55)" }}>Angsuran/bln ({fmt(pinRp)} × (1+{gadaiParams.persenAnggota}%) / {gForm.tenor})</span>
                                    <strong style={{ color:"#1d4ed8" }}>{fmt(angs)}</strong>
                                  </div>
                                </div>
                              )}
                              <div style={{ display:"flex", gap:8 }}>
                                <button onClick={()=>setGForm(f=>({...f,open:false}))} style={{ flex:1, background:"rgba(255,255,255,0.72)", border:"1px solid rgba(201,162,39,0.2)", borderRadius:8, padding:"9px", color:"rgba(101,67,14,0.7)", cursor:"pointer", fontSize:".82rem" }}>Batal</button>
                    ...
                    {detailLoading ? (
                      <p style={{ color:"rgba(101,67,14,0.35)", fontSize:".85rem", marginBottom:18 }}>Memuat...</p>
                    ) : detail.simpanan.length===0 ? (
                      <p style={{ color:"rgba(101,67,14,0.35)", fontSize:".85rem", marginBottom:18 }}>Belum ada simpanan.</p>
                    ) : (
                      <div style={{ border:"1px solid rgba(201,162,39,0.15)", borderRadius:9, overflow:"hidden", marginBottom:18 }}>
                        <div style={{ display:"flex", background:"rgba(255,255,255,0.72)", borderBottom:"1px solid rgba(201,162,39,0.12)", padding:"5px 12px" }}>
                          <span style={{ color:"rgba(101,67,14,0.35)", fontSize:".65rem", fontWeight:700, textTransform:"uppercase", flex:"0 0 120px" }}>Nominal</span>
                          <span style={{ color:"rgba(101,67,14,0.35)", fontSize:".65rem", fontWeight:700, textTransform:"uppercase", flex:"0 0 90px" }}>Tgl Transaksi</span>
                          <span style={{ color:"rgba(101,67,14,0.35)", fontSize:".65rem", fontWeight:700, textTransform:"uppercase", flex:1 }}>Keterangan</span>
                        </div>
                        {detail.simpanan.map((s:any, i:number)=>(
                          <div key={s.id} style={{ display:"flex", alignItems:"center", padding:"7px 12px", borderBottom: i < detail.simpanan.length-1 ? "1px solid rgba(201,162,39,0.1)" : "none", background:"rgba(255,255,255,0.01)" }}>
                            <span style={{ color:"#2D1B00", fontSize:".82rem", fontWeight:700, flex:"0 0 120px" }}>{fmt(s.amount)}</span>
                            <span style={{ color:"rgba(101,67,14,0.55)", fontSize:".78rem", flex:"0 0 90px" }}>{fmtDate(s.transaction_date||s.created_at)}</span>
                            <span style={{ color:"rgba(101,67,14,0.45)", fontSize:".78rem", flex:1 }}>{s.description||"-"}</span>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              {/* Recent Transactions */}
              <h3 style={{ color:"rgba(101,67,14,0.7)", fontSize:".8rem", fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", marginBottom:10 }}>5 Transaksi Terakhir</h3>
              {detailLoading ? (
                <p style={{ color:"rgba(101,67,14,0.35)", fontSize:".85rem" }}>Memuat...</p>
              ) : detail.transactions.length===0 ? (
                <p style={{ color:"rgba(101,67,14,0.35)", fontSize:".85rem" }}>Belum ada transaksi.</p>
              ) : (
                <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:18 }}>
                  {detail.transactions.map((tx:any)=>{
                    ...
                    return (
                      <div key={tx.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"rgba(255,255,255,0.72)", borderRadius:9, padding:"9px 14px", gap:8 }}>
                        <span style={{ color:"rgba(101,67,14,0.75)", fontSize:".83rem", flex:1 }}>{TX_LBL[tx.type]||tx.type}</span>
                        <span style={{ color:"rgba(101,67,14,0.45)", fontSize:".75rem" }}>{fmtDate(tx.transaction_date||tx.created_at)}</span>
                        <span style={{ color:"#8B6010", fontWeight:600, fontSize:".83rem" }}>{fmt(tx.amount)}</span>
                        <span style={{ color:STATUS_COLOR[tx.status]||"#fff", fontSize:".75rem", background:STATUS_BG[tx.status]||"rgba(201,162,39,0.12)", borderRadius:5, padding:"2px 8px" }}>{tx.status}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              {detail.savings.length>0 && (
                <>
                  <h3 style={{ color:"rgba(101,67,14,0.7)", fontSize:".8rem", fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", marginBottom:10 }}>Simpanan</h3>
                  <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:18 }}>
                    {detail.savings.map((s:any)=>(
                      <div key={s.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"rgba(255,255,255,0.72)", borderRadius:9, padding:"9px 14px" }}>
                        <span style={{ color:"rgba(101,67,14,0.75)", fontSize:".83rem" }}>{s.name}</span>
                        <span style={{ color:"#8B6010", fontSize:".83rem" }}>{s.current_gram}/{s.target_gram} gr</span>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", margin:"18px 0 10px" }}>
                <h3 style={{ color:"rgba(101,67,14,0.7)", fontSize:".8rem", fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", margin:0 }}>Beli / Cicilan Emas</h3>
                {!cForm.open && (
              ...
              {cForm.open && (() => { const cc = cicilanCalc(); return (
                <div style={{ background:"rgba(167,139,250,0.05)", border:"1px solid rgba(167,139,250,0.2)", borderRadius:10, padding:14, marginBottom:12, display:"flex", flexDirection:"column", gap:10 }}>
                  {cForm.err && <p style={{ color:"#991b1b", fontSize:".78rem", margin:0 }}>{cForm.err}</p>}
                  ...
                  {cc.row && (
                    <div style={{ background:"rgba(255,255,255,0.72)", borderRadius:9, padding:"10px 14px", display:"flex", flexDirection:"column", gap:6 }}>
                      {[
                        { label:"Harga emas", value:fmt(cc.row.harga), color:"rgba(101,67,14,0.7)" },
                        ...(cForm.type==="cicilan" ? [
                          { label:`Total cicilan (${cc.tenor} bln)`, value:fmt(cc.total), color:"rgba(101,67,14,0.7)" },
                          { label:"DP (uang muka)", value:fmt(cc.dp), color:"#1d4ed8" },
                          { label:"Angsuran / bln", value:fmt(cc.monthly), color:"#a78bfa" },
                        ] : [
                          { label:"Total bayar", value:fmt(cc.total), color:"#8B6010" },
                        ]),
                      ].map(r=>(
                        <div key={r.label} style={{ display:"flex", justifyContent:"space-between" }}>
                          <span style={{ color:"rgba(101,67,14,0.45)", fontSize:".8rem" }}>{r.label}</span>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    <button onClick={()=>setCForm(f=>({...f,open:false}))} style={{ flex:1, background:"rgba(255,255,255,0.72)", border:"1px solid rgba(201,162,39,0.2)", borderRadius:8, padding:"9px", color:"rgba(101,67,14,0.7)", cursor:"pointer", fontSize:".82rem" }}>Batal</button>
              ...
              {detail.installments.length===0 ? (
                <p style={{ color:"rgba(101,67,14,0.35)", fontSize:".83rem", marginBottom:8 }}>Belum ada cicilan.</p>
              ) : (
              ...
              {detail.gadai.length>0 && (
                <>
                  <h3 style={{ color:"rgba(101,67,14,0.7)", fontSize:".8rem", fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", margin:"18px 0 10px" }}>Gadai</h3>
                  <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                    {detail.gadai.map((g:any)=>(
                      <div key={g.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"rgba(255,255,255,0.72)", borderRadius:9, padding:"9px 14px" }}>
                        <span style={{ color:"rgba(101,67,14,0.75)", fontSize:".83rem" }}>Pinjaman {fmt(g.dana_cair)}</span>
                        <span style={{ color:"rgba(101,67,14,0.55)", fontSize:".83rem" }}>sisa {fmt(g.sisa_tagihan)} · {g.tenor} bln</span>
                        <span style={{ color:"#1d4ed8", fontSize:".75rem", textTransform:"capitalize" }}>{g.status}</span>
                      </div>
                    ))}
                  </div>
                </>
            ...
            >
              ...
              <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
                {[
                  ...
                  <div key={f.key}>
                    <label style={{ color:"rgba(101,67,14,0.55)", fontSize:".8rem", display:"block", marginBottom:6 }}>{f.label}</label>
