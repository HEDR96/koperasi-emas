  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24, maxWidth:900 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ color:"#2D1B00", fontSize:"1.4rem", fontWeight:700, margin:0 }}>Kelola Berita</h1>
          <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".85rem", margin:"4px 0 0" }}>Publikasikan berita & pengumuman koperasi</p>
        </div>
      ...
      <div style={{ background:"rgba(212,175,55,0.04)", border:"1px solid rgba(212,175,55,0.2)", borderRadius:16, padding:20 }}>
        ...
        {error && <p style={{ color:"#991b1b", fontSize:".82rem", marginBottom:10 }}>{error}</p>}
      ...
      {loading ? <p style={{ color:"rgba(101,67,14,0.35)" }}>Memuat...</p>
        : news.length === 0 ? <p style={{ color:"rgba(101,67,14,0.35)" }}>Belum ada berita.</p>
        : (
              ...
              <motion.div key={n.id} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }}
                ...
                <div style={{ flex:1, minWidth:200 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <p style={{ color:"#2D1B00", fontWeight:700, fontSize:".92rem", margin:0 }}>{n.title}</p>
                    {n.category && <span style={{ background:"rgba(96,165,250,0.12)", color:"#1d4ed8", borderRadius:6, padding:"2px 8px", fontSize:".7rem" }}>{n.category}</span>}
                  </div>
                  <p style={{ color:"rgba(101,67,14,0.4)", fontSize:".74rem", margin:"4px 0 0" }}>{fmtDate(n.created_at)}</p>
                </div>
                <div style={{ display:"flex", gap:8 }}>
                  <button onClick={()=>togglePublish(n)}
                    style={{ display:"flex", alignItems:"center", gap:5, background: n.is_published?"rgba(6,95,70,0.08)":"rgba(201,162,39,0.1)", border:`1px solid ${n.is_published?"rgba(52,211,153,0.25)":"rgba(201,162,39,0.2)"}`, borderRadius:8, padding:"6px 12px", color:n.is_published?"#34d399":"rgba(101,67,14,0.55)", cursor:"pointer", fontSize:".78rem", fontWeight:600 }}>
                  ...
                  {isMaster && (
                    <button onClick={()=>remove(n.id)} title="Hapus (master)"
                      style={{ background:"rgba(153,27,27,0.06)", border:"1px solid rgba(248,113,113,0.2)", borderRadius:8, padding:"6px 10px", color:"#991b1b", cursor:"pointer" }}>
