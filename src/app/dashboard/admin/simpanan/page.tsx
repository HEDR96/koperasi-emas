  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24, maxWidth:900 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ color:"#2D1B00", fontSize:"1.4rem", fontWeight:700, margin:0 }}>Input Simpanan Anggota</h1>
          <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".85rem", margin:"4px 0 0" }}>Catat setoran simpanan anggota</p>
        </div>
        <button onClick={()=>{loadRecent();loadPending();}} style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(212,175,55,0.1)", border:"1px solid rgba(212,175,55,0.25)", borderRadius:10, padding:"8px 14px", color:"#8B6010", cursor:"pointer", fontSize:".85rem" }}>
      ...
      <div>
        <p style={{ color:"rgba(101,67,14,0.55)", fontSize:".78rem", fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", margin:"0 0 12px", display:"flex", alignItems:"center", gap:8 }}>
          <Clock style={{ width:13, height:13, color:"#92400e" }} /> Menunggu Persetujuan
          {pending.length > 0 && <span style={{ background:"rgba(146,64,14,0.1)", color:"#92400e", borderRadius:20, padding:"2px 8px", fontSize:".72rem" }}>{pending.length}</span>}
        </p>
        {loadingPending ? (
          <p style={{ color:"rgba(101,67,14,0.3)", fontSize:".85rem" }}>Memuat...</p>
        ) : pending.length === 0 ? (
          <p style={{ color:"rgba(101,67,14,0.3)", fontSize:".85rem" }}>Tidak ada setoran yang menunggu persetujuan.</p>
        ) : (
            ...
            {pending.map(r => (
              <div key={r.id} style={{ background:"rgba(251,191,36,0.04)", border:"1px solid rgba(251,191,36,0.2)", borderRadius:12, padding:"12px 16px", display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
                <div style={{ flex:1, minWidth:160 }}>
                  <p style={{ color:"#2D1B00", fontWeight:600, fontSize:".88rem", margin:"0 0 3px" }}>{r.profiles?.name||"-"}</p>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        ...
        <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }}
          ...
          {saved && <div style={{ background:"rgba(6,95,70,0.08)", border:"1px solid rgba(52,211,153,0.2)", borderRadius:10, padding:"10px 14px", color:"#065f46", fontSize:".82rem", display:"flex", alignItems:"center", gap:8 }}><CheckCircle style={{width:14,height:14}}/> Simpanan tersimpan!</div>}
          ...
          <div>
            <label style={{ color:"rgba(101,67,14,0.45)", fontSize:".78rem", display:"block", marginBottom:7 }}>Nominal (Rp) *</label>
            <RupiahInput value={form.amount} onValueChange={v=>setForm(p=>({...p,amount:v}))} style={inp} />
          </div>

          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            <div>
              <label style={{ color:"rgba(101,67,14,0.45)", fontSize:".78rem", display:"block", marginBottom:7 }}>Tanggal Setoran</label>
              <input type="date" value={form.tgl} onChange={e=>setForm(p=>({...p,tgl:e.target.value}))} style={inp} />
            </div>
            <div>
              <label style={{ color:"rgba(101,67,14,0.45)", fontSize:".78rem", display:"block", marginBottom:7 }}>Keterangan</label>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <p style={{ color:"rgba(101,67,14,0.55)", fontSize:".78rem", fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", margin:0 }}>Simpanan Terbaru</p>
          {loading ? <p style={{ color:"rgba(101,67,14,0.3)", fontSize:".85rem" }}>Memuat...</p>
            : recent.length === 0 ? <p style={{ color:"rgba(101,67,14,0.3)", fontSize:".85rem" }}>Belum ada data.</p>
            : recent.map(r => (
              <div key={r.id} style={{ background:"rgba(255,255,255,0.72)", border:"1px solid rgba(201,162,39,0.12)", borderRadius:12, padding:"12px 16px" }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between" }}>
                  <span style={{ color:"rgba(101,67,14,0.6)", fontSize:".8rem" }}>{r.profiles?.name||"-"} · Tgl setor {fmtTgl(r.transaction_date || r.created_at)}</span>
                  <span style={{ color:"#2D1B00", fontWeight:700, fontSize:".85rem" }}>{fmt(r.amount)}</span>
                </div>
                <div style={{ marginTop:6, paddingTop:6, borderTop:"1px solid rgba(201,162,39,0.12)", display:"flex", justifyContent:"space-between", gap:8, flexWrap:"wrap" }}>
                  <span style={{ color:"rgba(101,67,14,0.45)", fontSize:".7rem" }}>Diinput oleh: <span style={{ color:"#a78bfa" }}>{staff[r.recorded_by] || "—"}</span></span>
                  <span style={{ color:"rgba(101,67,14,0.35)", fontSize:".7rem" }}>Diinput: {fmtTglJam(r.created_at)}</span>
                </div>
              </div>
            ))}
        </div>
