  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24, maxWidth:900 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ color:"#2D1B00", fontSize:"1.4rem", fontWeight:800, margin:0, letterSpacing:"-.02em" }}>Voucher</h1>
          <p style={{ color:"rgba(101,67,14,0.35)", fontSize:".82rem", margin:"3px 0 0" }}>Kelola voucher potongan harga produk & angsuran</p>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={load} style={{ width:38, height:38, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(255,255,255,0.72)", border:"1px solid rgba(201,162,39,0.18)", borderRadius:10, color:"rgba(101,67,14,0.45)", cursor:"pointer" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
        {[
          ...
          { label:"Aktif",         value:aktifVoucher, color:"#065f46" },
          { label:"Nonaktif",      value:totalVoucher-aktifVoucher, color:"rgba(101,67,14,0.3)" },
        ].map(s => (
      ...
      {loading ? (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>{[1,2,3].map(i=><div key={i} style={{ height:64, borderRadius:12, background:"rgba(255,255,255,0.72)" }} />)}</div>
      ) : list.length === 0 ? (
        <div style={{ textAlign:"center", padding:"56px 20px", background:"rgba(255,255,255,0.72)", border:"1px dashed rgba(255,255,255,0.08)", borderRadius:16 }}>
          ...
          <p style={{ color:"rgba(101,67,14,0.3)", margin:0, fontSize:".88rem" }}>Belum ada voucher</p>
        </div>
      ) : (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {list.map((v, i) => (
            <motion.div key={v.id} layout initial={{ opacity:0, x:-8 }} animate={{ opacity:1, x:0 }} transition={{ delay:i*.04 }}
              style={{ display:"flex", alignItems:"center", gap:14, background:"rgba(255,255,255,0.72)", border:`1px solid ${v.is_active?"rgba(6,95,70,0.1)":"rgba(201,162,39,0.12)"}`, borderRadius:14, padding:"12px 16px" }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:5, flexWrap:"wrap" }}>
                  <p style={{ color:"#2D1B00", fontWeight:800, fontSize:".9rem", margin:0, letterSpacing:".03em" }}>{v.code}</p>
                  <span style={{ fontSize:".66rem", fontWeight:700, borderRadius:20, padding:"2px 8px", background:v.is_active?"rgba(6,95,70,0.09)":"rgba(201,162,39,0.1)", color:v.is_active?"#34d399":"rgba(101,67,14,0.3)", border:`1px solid ${v.is_active?"rgba(52,211,153,0.25)":"rgba(201,162,39,0.15)"}` }}>
                ...
                <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
                  <span style={{ display:"flex", alignItems:"center", gap:3, fontSize:".72rem", color:"#065f46", background:"rgba(52,211,153,0.08)", borderRadius:5, padding:"2px 8px", fontWeight:600 }}>
                  ...
                  {v.description && <span style={{ color:"rgba(101,67,14,0.45)", fontSize:".76rem" }}>{v.description}</span>}
                </div>
                <p style={{ color:"rgba(101,67,14,0.3)", fontSize:".7rem", margin:"5px 0 0" }}>Dibuat {fmtDt(v.created_at)}</p>
              </div>
              <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                ...
                <button onClick={() => toggle(v)} style={{ width:32, height:32, display:"flex", alignItems:"center", justifyContent:"center", background:v.is_active?"rgba(52,211,153,0.07)":"rgba(255,255,255,0.04)", border:`1px solid ${v.is_active?"rgba(52,211,153,0.25)":"rgba(201,162,39,0.2)"}`, borderRadius:8, color:v.is_active?"#34d399":"rgba(101,67,14,0.35)", cursor:"pointer" }}>
                  {v.is_active?<ToggleRight style={{ width:14, height:14 }} />:<ToggleLeft style={{ width:14, height:14 }} />}
                </button>
                {isMaster && (
                  <button onClick={() => remove(v.id)} style={{ width:32, height:32, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(248,113,113,0.06)", border:"1px solid rgba(248,113,113,0.15)", borderRadius:8, color:"#991b1b", cursor:"pointer" }}>
            ...
            <motion.div initial={{ opacity:0, scale:.95, y:16 }} animate={{ opacity:1, scale:1, y:0 }} exit={{ opacity:0, scale:.95, y:16 }} transition={{ type:"spring", stiffness:300, damping:28 }}
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"18px 22px 14px", borderBottom:"1px solid rgba(201,162,39,0.15)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  ...
                  <span style={{ color:"#2D1B00", fontWeight:700, fontSize:".95rem" }}>{editId?"Edit Voucher":"Voucher Baru"}</span>
                </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                    {([["produk","Harga Produk"],["angsuran","Angsuran (Cicilan)"]] as const).map(([val,label]) => (
                      <button key={val} onClick={() => setForm(p=>({...p,target:val}))}
                        style={{ padding:11, borderRadius:10, cursor:"pointer", fontSize:".84rem", fontWeight:600, transition:"all .15s", border:`1px solid ${form.target===val?"rgba(212,175,55,0.45)":"rgba(201,162,39,0.15)"}`, background:form.target===val?"rgba(212,175,55,0.1)":"rgba(255,255,255,0.03)", color:form.target===val?G:"rgba(101,67,14,0.4)" }}>
                        {label}
                      </button>
                    ))}
                  </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                    {([["percent","Persen (%)"],["fixed","Nominal (Rp)"]] as const).map(([val,label]) => (
                      <button key={val} onClick={() => setForm(p=>({...p,discount_type:val}))}
                        style={{ padding:11, borderRadius:10, cursor:"pointer", fontSize:".84rem", fontWeight:600, transition:"all .15s", border:`1px solid ${form.discount_type===val?"rgba(212,175,55,0.45)":"rgba(201,162,39,0.15)"}`, background:form.discount_type===val?"rgba(212,175,55,0.1)":"rgba(255,255,255,0.03)", color:form.discount_type===val?G:"rgba(101,67,14,0.4)" }}>
                        {label}
                      </button>
                    ))}
                  </div>
