  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24, maxWidth:820 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ color:"#fff", fontSize:"1.4rem", fontWeight:700, margin:0 }}>Hubungi Anggota</h1>
          <p style={{ color:"rgba(255,255,255,0.4)", fontSize:".85rem", margin:"4px 0 0" }}>Kontak anggota langsung via WhatsApp</p>
        </div>
      ...
      {loading ? <p style={{ color:"rgba(255,255,255,0.3)" }}>Memuat...</p>
        : filtered.length === 0 ? <p style={{ color:"rgba(255,255,255,0.3)" }}>Tidak ada anggota.</p>
        : (
            ...
            {filtered.map((m,i)=>{
              ...
              return (
                <motion.div key={m.id} initial={{ opacity:0, y:6 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*.02 }}
                  style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:12, padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                    <div style={{ width:36, height:36, borderRadius:9, background:"rgba(52,211,153,0.12)", display:"flex", alignItems:"center", justifyContent:"center", color:"#34d399", fontWeight:700, fontSize:".85rem" }}>
                      {m.name?.[0]?.toUpperCase()||"M"}
                    </div>
                    <div>
                      <p style={{ color:"#fff", fontWeight:600, fontSize:".88rem", margin:0 }}>{m.name}</p>
                      <p style={{ color:"rgba(255,255,255,0.4)", fontSize:".76rem", margin:"2px 0 0" }}>{m.phone||"Tidak ada nomor"}</p>
                    </div>
                  </div>
                  {wa ? (
                    ...
                    <span style={{ display:"flex", alignItems:"center", gap:6, color:"rgba(255,255,255,0.25)", fontSize:".8rem" }}>
