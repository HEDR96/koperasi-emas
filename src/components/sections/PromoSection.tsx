...
function ProdukCard({ item, index, onAddCart, inCart }: { item: ProdukItem; index: number; onAddCart: (item: ProdukItem) => void; inCart?: boolean }) {
  ...
  const habis = item.stok != null && item.stok <= 0;
...
export default function PromoSection() {
  ...
  const rekomendasi = items.filter(it => it.price && !cartIds.has(it.id) && !(it.stok!=null&&it.stok<=0)).slice(0,3);

  function buildWaMessage() {
    const lines = [`Halo, saya *${custName||"(nama belum diisi)"}* ingin memesan produk berikut:`, ""];
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))", gap:20 }}>
            {items.map((item,i) => (
              <div key={item.id} style={{ position:"relative" }}>
                <ProdukCard item={item} index={i} onAddCart={addToCart} inCart={cartIds.has(item.id)} />
                <AnimatePresence>
                  {justAdded===item.id && (
