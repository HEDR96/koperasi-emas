...
export default function HargaViewer() {
  ...
  const TABS: { id: Tab; label: string; color: string; border: string }[] = [
    { id:"emas",    label:"Harga Emas",    color:"#8B6010", border:"rgba(201,162,39,0.4)" },
    { id:"cicilan", label:"Harga Cicilan", color:"#6d28d9", border:"rgba(109,40,217,0.35)" },
    { id:"buyback", label:"Harga Buyback", color:"#065f46", border:"rgba(6,95,70,0.35)" },
  ];
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:24, maxWidth:760 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ color:"#2D1B00", fontSize:"1.4rem", fontWeight:700, margin:0 }}>Harga Emas</h1>
          <p style={{ color:"rgba(101,67,14,0.45)", fontSize:".85rem", margin:"4px 0 0" }}>Daftar harga emas, cicilan, dan buyback koperasi emas</p>
        </div>
        <button onClick={load} style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(201,162,39,0.1)", border:"1px solid rgba(201,162,39,0.25)", borderRadius:10, padding:"8px 14px", color:"#8B6010", cursor:"pointer", fontSize:".85rem" }}>
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ padding:"8px 20px", borderRadius:10, fontSize:".88rem", fontWeight:600, cursor:"pointer", transition:"all .2s",
              border: tab === t.id ? `1px solid ${t.border}` : "1px solid rgba(201,162,39,0.18)",
              background: tab === t.id ? `rgba(255,255,255,0.72)` : "rgba(255,255,255,0.5)",
              color: tab === t.id ? t.color : "rgba(101,67,14,0.5)" }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <>
        {tab === "emas" && (
          <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }}
            style={{ background:"rgba(255,255,255,0.72)", border:"1px solid rgba(201,162,39,0.22)", borderRadius:16, overflow:"hidden" }}>
            <div style={{ padding:"14px 20px", borderBottom:"1px solid rgba(201,162,39,0.15)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <p style={{ color:"#8B6010", fontWeight:700, fontSize:".85rem", margin:0 }}>Harga Beli Emas (per berat)</p>
              <span style={{ color:"rgba(101,67,14,0.45)", fontSize:".75rem" }}>harga koperasi emas</span>
            </div>
              ...
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead><tr style={{ borderBottom:"1px solid rgba(201,162,39,0.12)" }}>
                  {["Berat", "Harga", "Terakhir Update"].map(h=>(
                    <th key={h} style={{ padding:"10px 18px", textAlign:"left", color:"rgba(101,67,14,0.4)", fontSize:".72rem", fontWeight:600, textTransform:"uppercase" }}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {hargaEmas.map((r,i)=>(
        ...
        {tab === "cicilan" && (
          <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }}
            style={{ background:"rgba(255,255,255,0.72)", border:"1px solid rgba(109,40,217,0.2)", borderRadius:16, overflow:"hidden" }}>
            <div style={{ padding:"14px 20px", borderBottom:"1px solid rgba(201,162,39,0.12)" }}>
              <p style={{ color:"#6d28d9", fontWeight:700, fontSize:".85rem", margin:0 }}>Paket Cicilan Tersedia</p>
            ...
            {cicilan.length === 0 ? <p style={{ padding:"20px", color:"rgba(101,67,14,0.35)", fontSize:".85rem" }}>Belum ada data harga emas</p> : (
              <div style={{ overflowX:"auto" }}>
                <table style={{ width:"100%", borderCollapse:"collapse" }}>
                  <thead><tr style={{ borderBottom:"1px solid rgba(201,162,39,0.12)" }}>
                    {["Gram", "Harga", ...CICILAN_TENORS.map(t=>`${t} bln`)].map(h=>(
                      <th key={h} style={{ padding:"10px 16px", textAlign:"left", color:"rgba(101,67,14,0.4)", fontSize:".72rem", fontWeight:600, textTransform:"uppercase", whiteSpace:"nowrap" }}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {cicilan.map((r,i)=>(
        ...
        {tab === "buyback" && (
          <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }}
            style={{ background:"rgba(255,255,255,0.72)", border:"1px solid rgba(6,95,70,0.2)", borderRadius:16, overflow:"hidden" }}>
            <div style={{ padding:"14px 20px", borderBottom:"1px solid rgba(201,162,39,0.12)" }}>
